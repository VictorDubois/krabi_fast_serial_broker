...
public:
    SimpleBrokerSTM32()
      : Node("simple_broker_stm32")
    {
        subscription_cmd_vel_ = this->create_subscription<geometry_msgs::msg::Twist>(
          "cmd_vel",
          10,
          std::bind(&SimpleBrokerSTM32::callback_cmd_vel, this, std::placeholders::_1));

        subscription_motors_cmd_ = this->create_subscription<krabi_msgs::msg::MotorsCmd>(
          "motors_cmd",
          10,
          std::bind(&SimpleBrokerSTM32::callback_motors_cmd, this, std::placeholders::_1));

        subscription_motors_parameters_
          = this->create_subscription<krabi_msgs::msg::MotorsParameters>(
            "motors_parameters",
            10,
            std::bind(&SimpleBrokerSTM32::callback_motors_parameters, this, std::placeholders::_1));

        subscription_enable_motor_ = this->create_subscription<std_msgs::msg::Bool>(
          "enable_motor",
          10,
          std::bind(&SimpleBrokerSTM32::callback_enable_motors, this, std::placeholders::_1));
...
#define CRC_INITIAL_VALUE 0xFFFF
    ...
    void callback_motors_parameters(const krabi_msgs::msg::MotorsParameters::SharedPtr msg)
    {
        ...
        // RCLCPP_WARN_STREAM(this->get_logger(),  float_array_to_send[0] << ", " <<
        // float_array_to_send[1] << ", " << float_array_to_send[2]);
    ...
    void write_float_to_serial(char channel_name, const std::vector<float>& float_values)
    {
        ...
        std::string message = std::string(1, channel_name)
                              + std::string(reinterpret_cast<char*>(hex_values), sizeof(hex_values))
                              + "\n";
        ...
        else
        {
            ...
            // std::cout << "Sending: " << channel_name <<
            // std::string(reinterpret_cast<char*>(hex_values), sizeof(hex_values)) << std::endl;
            // std::cout.flush();
    ...
    void get_float(const std::string& a_string, int a_beginning, float& float_value)
    {
        ...
        uint32_t float_hex
          = strtoul(hex_value.c_str(), NULL, 16); // Convert hexadecimal string to unsigned long
        // float_value = 0.f;
        // uint32_t float_hex = std::stoul(hex_value, nullptr, 16); // Convert hexadecimal string to
        // integer
    ...
    void read_serial()
    {
        ...
        if (line.length() >= 2 + 8 * 5 + 2 && line.substr(0, 2) == "o:") // Ensure CRC is included
        {
            ...
            std::string crc_str
              = line.substr(line.length() - 3, 2); // Assuming CRC is two bytes long
            uint16_t received_crc = 0;
            memcpy(&received_crc, crc_str.c_str(), sizeof(uint16_t));

            // Compute CRC for the received data
            uint16_t computed_crc = calculate_crc(reinterpret_cast<const uint8_t*>(line.c_str()),
                                                  line.length() - 3); // Excluding CRC bytes
            ...
            else
            {
                ...
                RCLCPP_ERROR_STREAM(this->get_logger(),
                                    received_crc << " != " << computed_crc << std::endl);
            }
        }
        else
        {
    ...
    rclcpp::Subscription<krabi_msgs::msg::MotorsParameters>::SharedPtr
      subscription_motors_parameters_;
